        ...
        foreach ($vars as $i => $var) {
            if (isset($varNames[$i])) {
                ...
                if (preg_match('/^[$][a-zA-Z_][a-zA-Z0-9_]*$/', $raw)) {
                    $output .= $raw . ' = ' . craft_custom_var_dump($var) . "\n";
                } else {
                    $output .= craft_custom_var_dump($var) . "\n";
                }
            } else {
                $output .= craft_custom_var_dump($var) . "\n";
            }
...
/**
 * Custom var_dump function with better formatting and recursion detection.
 * 
 ...
 * @return string
 */
function craft_custom_var_dump($var, $indent = 0, &$references = [])
{
