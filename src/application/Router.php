...
/*
 *| Route Class to handle HTTP routing in the application.
 *| 
...
class Router
{
    ...
    public static function name(string $name): self
    {
        if (self::$lastRegisteredRoute) {
            ...
            self::$routeNames[$name] = [$method, $path];
        } else {
            self::$groupContext['name'] = $name;
        }
        return new static();
    }
    ...
    public static function action(callable $callback): self
    {
        $callback();
    ...
    public static function runTest($testValue = '1')
    {
        ...
        echo str_pad('Result', 8) . str_pad('Method', 8) . str_pad('Test Path', 30) . "Route\n";
        echo str_repeat('-', 60) . "\n";
        foreach ($results as $r) {
            echo str_pad($r['icon'] . ' ' . $r['result'], 8)
                . str_pad($r['method'], 8)
                . str_pad($r['test_path'], 30)
                . $r['path'] . "\n";
        }
    }
